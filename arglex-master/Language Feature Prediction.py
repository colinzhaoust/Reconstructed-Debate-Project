...
# glove_file = datapath('C:\\Users\赵欣然\Desktop\Assignment1_resources\glove.42B.300d.txt')
# tmp_file = get_tmpfile('test.txt')
# from gensim.scripts.glove2word2vec import glove2word2vec
# glove2word2vec(glove_file, tmp_file)
# google_model = KeyedVectors.load_word2vec_format(tmp_file)
# print("model loaded")
# sample = google_model.get_vector("man")
# print(sample)
# print(len(sample))
...
print("Number of all users", number_of_users)


def get_target_users(target, debates, users):
    user_to_language = dict()

    for debate in debates.values():
        if target.lower() in debate["title"].lower():

            # commented users
            # for comment in debate["comments"]:
            #     user_name = comment["user_name"]
            #     text = comment["comment_text"]
            #     if users.get(user_name) and len(text) > 0:
            #         if not user_to_language.get(user_name):
            #             user_to_language[user_name] = [text]
            #         else:
            #             user_to_language[user_name].append(text)

            # debaters
            for debater in [debate["participant_1_name"], debate["participant_2_name"]]:

                 if debater == debate["participant_1_name"]:
                     side = debate["participant_1_position"]
                 else:
                     side = debate["participant_2_position"]

                 if users.get(debater):
                     for round in debate["rounds"]:
                         for argument in round:
                            print(argument["text"])
                            if argument["side"] == side:
                                if len(argument["text"]) > 0:
                                    if not user_to_language.get(debater):
                                        user_to_language[debater] = [argument["text"]]
                                    else:
                                        user_to_language[debater].append(argument["text"])
    return user_to_language


target = "Abortion"
user_language_dict = get_target_users(target, debates, users)
print("Number of valid user_text pairs: ", len(user_language_dict))

# breakpoint()

def speech_tokenize(corpus_path):
    # import the speech corpus
    corpus = open(corpus_path, 'r', encoding='UTF-8').readlines()
    output = []
    for line in corpus:
...
def speech_vectorize(token_list, w2v_model):
    unknown = 0
    intial_matrix = w2v_model.get_vector("man")
    vector_len = len(intial_matrix)
    intial_matrix = np.where(intial_matrix, 0., 0.)
    valid_count = 0

    for token in token_list:
        try:
            intial_matrix += w2v_model.get_vector(token)
            valid_count += 1
        except:
            unknown += 1

    output_matrix = np.where(intial_matrix, intial_matrix/valid_count, intial_matrix/valid_count)

    return output_matrix, vector_len


def get_language_vector(text_list):
    ...
    # sentence length
    # try:
    #     ave_sent_length = len(word_tokenize(all_text))/len(sent_tokenize(all_text))
    #     language_vec.extend(ave_sent_length)
    # except:
    #     language_vec.extend([0.0])
    ...
    try:
        ...
        language_vec.append(subjectivity)
        language_vec.append(polarity)
    except:
        language_vec.extend([0.0 * 2])

    # word embedding
    try:
        speech_vector, vector_len = speech_vectorize(all_text, google_model)
        language_vec.extend(speech_vector)
    except:
        language_vec.extend([0.0 * 300])

    return language_vec


def get_training_data(user_language_dict, users, target_big_issue):
    possible_choices = ["Pro", "Con", "Und", "N/O"]
    xTr = []
    yTr = []
    number_per_choice = dict()
    for choice in possible_choices:
        number_per_choice[choice] = 0

    for user, text in user_language_dict.items():
        if users.get(user):
            this_user = users.get(user)
            if not this_user["big_issues_dict"][target_big_issue] in possible_choices:
                continue

            choice = this_user["big_issues_dict"][target_big_issue]
            number_per_choice[choice] += 1
            label = possible_choices.index(choice)

            lang_vec = get_language_vector(text)

            expected_length = 307
            if len(lang_vec) == expected_length and lang_vec != [0.0 * expected_length]:
                xTr.append(lang_vec)
                yTr.append(label)

    return xTr, yTr, number_per_choice


raw_xTr, raw_yTr, number_per_choice = get_training_data(user_language_dict, users, target)
print(len(raw_xTr), len(raw_yTr))

# balance the data set
min = 80000
for number in number_per_choice.values():
    if min > number:
        min = number
    print(number)

print("Data for each label:", min)

try:
    training_data = zip(raw_xTr, raw_yTr)
    ...
    yTr = raw_yTr
# count = dict()
# for i in range(len(raw_yTr)):
#     label = raw_yTr[i]
#     if not count.get(label):
#         count[label] = 1
#         xTr.append(raw_xTr[i])
#         yTr.append(raw_yTr[i])
#     else:
#         count[label] += 1
#         if count[label] <= min:
#             xTr.append(raw_xTr[i])
...
clf = LogisticRegression(random_state=0, solver='lbfgs').fit(training_x, training_y)
...
# unbalanced_xTr, unbalanced_yTr = get_training_set(valid_users, debates, "Global Warming Exists", big_issues_list, criterion)
#
# xTr = []
# yTr = []
# number_of_0 = 0
...
for i in range(int(len(tqdm(test_x)))):
    ...
    baseline += int((gold_output == 0))
    predictions += 1

print(max_score)
accuracy = correct / predictions
baseline_list.append(baseline / predictions)
assert 0 <= accuracy <= 1
